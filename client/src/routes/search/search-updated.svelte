<script lang="ts">
    ...
    async function getNearbyListings() {
        ...
        if (error) {
            ...
            listings = data;
            applyChargerTypeFilter(); // Apply charger type filter
    ...
    function applyChargerTypeFilter() {
        ...
        if (listings.length < 20 && selectedChargerType !== 'all') { 
            // Fetch more results if too few are found
            fetchMoreResults();
        }
    }
    ...
    async function fetchMoreResults() {
        ...
        if (!error) {
            listings = data.filter(listing => listing.charger_type === selectedChargerType);
        }
    }
...
<label class="label flex mb-2">
    <GeocodeSearchbar
        ...
        n={1}
    />
    <button type="button" class="btn m-2 variant-filled" on:click={getNearbyListings}>
